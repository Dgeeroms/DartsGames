...
class ViewModelFactory(private val application: Application, private val gameDao: GameDao): ViewModelProvider.Factory  {
    ...
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        ...
        if (modelClass.isAssignableFrom(ContinueGameViewModel::class.java)) {
            return ContinueGameViewModel(application, gameDao) as T
        }

        if (modelClass.isAssignableFrom(ScoresViewModel::class.java)) {
            return ScoresViewModel(application, gameDao) as T
        }
